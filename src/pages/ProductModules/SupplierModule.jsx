...
export default function SupplierModule() {
  ...
  const handleToggleAccess = async (row) => {
    ...
    try {
      await Supplierservice.updateSuppliers({ status: newStatus }, row.id);
      toast({
        variant: "success",
        title: "Supplier App Management",
  ...
  const handleDelete = async (rowOrId) => {
    ...
    try {
      ...
      if (res) {
        toast({
          variant: "success",
          title: res?.data?.message || "Supplier App Management",
  ...
  return (
    <div className="grid gap-4 lg:gap-6">
      <div className="flex items-center justify-between gap-2">
        <h3 className="h4-bold">Supplier App Management</h3>
