...
export default function ClientModule() {
  ...
  const getList = async (country) => {
    setLoader(true);
    try {
      const res = await Buyerservice.getList(country);
      let rawItems = [];
  ...
  const handleToggleAccess = async (row) => {
    ...
    try {
      await Buyerservice.updateBuyer(row.id, { ...row, status: newStatus });
      toast({
        variant: "success",
        title: "Client App Management",
  ...
  const handleDelete = async (rowOrId) => {
    ...
    try {
      ...
      if (res) {
        toast({
          variant: "success",
          title: res?.data?.message || "Client App Management",
  ...
  return (
    <div className="grid gap-4 lg:gap-6">
      <div className="flex items-center justify-between gap-2">
        <h3 className="h4-bold">Client App Management</h3>
