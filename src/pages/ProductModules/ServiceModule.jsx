...
export default function ServiceModule() {
  ...
  const handleToggleAccess = async (row) => {
    ...
    try {
      await Careerservice.updateCareer(row.id, { ...row, status: newStatus });
      toast({
        variant: "success",
        title: "Service App Management",
  ...
  const handleDelete = async (rowOrId) => {
    ...
    try {
      ...
      if (res) {
        toast({
          variant: "success",
          title: res?.data?.message || "Service App Management",
  ...
  return (
    <div className="grid gap-4 lg:gap-6">
      <div className="flex items-center justify-between gap-2">
        <h3 className="h4-bold">Service App Management</h3>
