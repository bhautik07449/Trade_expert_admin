    ...
    { field: "SrNo", headerName: "SrNo", flex: 1 },
    {
        ...
        renderCell: ({ row }) => (
            <img
                ...
                alt={row?.title}
                className="h-52 w-full object-cover rounded p-0.5"
            />
        )
    },
...
export default function Project() {
    ...
    const getData = async (country) => {
        try {
            const res = await IRProjectservice.getList(country);
            if (res) {
        ...
        try {
            ...
            if (res) {
                ...
                toast({
                    ...
                    description: res?.data?.message || "Project has been deleted successfully",
                });
            }
        } catch (error) {
            toast({
    ...
    const handleEdit = (row) => {
        navigate(`/website-management/section-management/ir_project/edit/${row.id}`)
    }
            ...
            <Card className="p-4 grid gap-4 lg:gap-6">
                <div className="flex items-center justify-end gap-4">
                    <div className="flex gap-3 items-center">
                        <Button className="flex items-center gap-2" onClick={() => navigate('/website-management/section-management/ir_project/add')}>
